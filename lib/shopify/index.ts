...
// DEFAULT_NAVIGATION moved to lib/constants/navigation.ts
...
export async function getNavigation(): Promise<Navigation> {
  ...
  const navigationCategories = removeEdgesAndNodes(
    res.body?.data?.navigationCategories,
  );
  ...
  if (navigationPages?.fields) {
    const pagesField = navigationPages.fields.find(
      (f: any) => f.key === "pages",
    );
    if (pagesField) {
