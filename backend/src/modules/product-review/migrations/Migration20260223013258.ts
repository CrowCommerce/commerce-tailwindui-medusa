...
export class Migration20260223013258 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "review_image" ("id" text not null, "url" text not null, "sort_order" integer not null default 0, "review_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "review_image_pkey" primary key ("id"));`);
    ...
    this.addSql(`create table if not exists "review_response" ("id" text not null, "content" text not null, "review_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "review_response_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table if exists "review_response" add constraint "review_response_review_id_foreign" foreign key ("review_id") references "review" ("id") on update cascade;`);
  }
