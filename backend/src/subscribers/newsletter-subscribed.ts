...
export default async function newsletterSubscribedHandler({
  ...
  try {
    ...
    Sentry.captureException(error, { tags: { subscriber: "newsletter_subscribed", step: "resend_sync", subscriber_id: data.id } })
    logger.warn(
  ...
  if (data.isNewSubscriber) {
    try {
      ...
      Sentry.captureException(error, { tags: { subscriber: "newsletter_subscribed", step: "welcome_email", subscriber_id: data.id } })
      logger.warn(
    ...
    try {
      ...
      Sentry.captureException(error, { tags: { subscriber: "newsletter_subscribed", step: "welcome_back_email", subscriber_id: data.id } })
      logger.warn(
