...
const COMPONENTS_PATH = path.resolve(__dirname, '../../../../Resources/TailwindUI/tailwindplus-components.json');
const SEED_DATA_PATH = path.resolve(__dirname, 'tailwindui-products.json');

// Generous path - try relative to CrowCommerce root too
const ALT_COMPONENTS_PATH = '/Users/itsjusteric/CrowCommerce/Resources/TailwindUI/tailwindplus-components.json';

console.log('🔍 Phase 1: Extracting all image URLs from TailwindUI components...\n');

let componentsPath = fs.existsSync(COMPONENTS_PATH) ? COMPONENTS_PATH : ALT_COMPONENTS_PATH;
if (!fs.existsSync(componentsPath)) {
  console.error('Cannot find tailwindplus-components.json at:\n  ' + COMPONENTS_PATH + '\n  ' + ALT_COMPONENTS_PATH);
  process.exit(1);
}
...
function extractFromCode(code, componentName) {
  ...
  const urlRegex = /https?:\/\/tailwindcss\.com\/plus-assets\/img\/ecommerce-images\/[^\s"'`)\]},]+/g;
  ...
  matches.forEach(url => {
    ...
    const context = code.substring(Math.max(0, idx - 300), idx + url.length + 300);
...
Object.entries(categories).sort((a,b) => b[1].length - a[1].length).forEach(([cat, urls]) => {
...
const productNamePatterns = {
  ...
  'leatherbound-daily-journal': ['Leatherbound Daily Journal', 'leatherbound daily journal'],
...
seedData.products.forEach(p => {
  ...
  if (p.images) p.images.forEach(img => { if (typeof img === 'string') urls.add(img); else if (img?.url) urls.add(img.url); });
...
function findGalleries(obj, componentPath = '') {
  ...
  for (const [key, val] of Object.entries(obj)) {
    if (typeof val === 'string' && val.includes('ecommerce-images') && val.includes('images')) {
      ...
      while ((match = imageArrayRegex.exec(val)) !== null) {
        ...
        const imgUrls = [];
        const urlMatches = arrayContent.match(/https?:\/\/tailwindcss\.com\/plus-assets\/img\/ecommerce-images\/[^\s"'`)\]},]+/g);
        if (urlMatches && urlMatches.length > 1) {
          // Find which product this gallery belongs to
          const nearbyText = val.substring(Math.max(0, match.index - 500), match.index);
          let productHandle = null;
          
          for (const [handle, names] of Object.entries(productNamePatterns)) {
            for (const name of names) {
              if (nearbyText.includes(name) || val.substring(match.index, match.index + 500).includes(name)) {
      ...
      const productBlockRegex = /(?:name|title)\s*:\s*['"]([^'"]+)['"]\s*,[\s\S]{0,500}?images\s*:\s*\[([^\]]+)\]/g;
      while ((match = productBlockRegex.exec(val)) !== null) {
        const productName = match[1];
        const imagesBlock = match[2];
        const urls = imagesBlock.match(/https?:\/\/tailwindcss\.com\/plus-assets\/img\/ecommerce-images\/[^\s"'`)\]},]+/g);
        if (urls && urls.length > 1) {
          ...
          for (const p of seedData.products) {
            if (p.title.toLowerCase() === productName.toLowerCase() || 
...
console.log(`Assigned: ${assignedUrls.size}, Unassigned product-relevant: ${unassigned.length}\n`);

// ─── Output: Write enriched data ──────────────────────────────────────────────
console.log('📊 Final image counts per product:\n');

const enrichedProducts = seedData.products.map(p => {
  const images = productImages.has(p.handle) ? [...productImages.get(p.handle)] : [p.thumbnail];
...
Object.entries(imageCounts).sort((a,b) => Number(a[0]) - Number(b[0])).forEach(([count, products]) => {
  console.log(`  ${count} images: ${products} products`);
});

console.log('\nProducts with 2+ images:');
enrichedProducts.filter(p => p.images.length >= 2).forEach(p => {
  console.log(`  ${p.handle}: ${p.images.length} images`);
  p.images.forEach(u => console.log(`    ${typeof u === 'string' ? u.split('/').pop() : u}`));
});
...
const enrichedPath = path.resolve(__dirname, 'tailwindui-products-enriched.json');
