...
| Search | Meilisearch, React InstantSearch | `meilisearch@0.56.0`, `@meilisearch/instant-meilisearch@0.30.0`, `react-instantsearch@7.28.0` | Search command palette, faceted search, backend indexing |
...
CrowCommerce is organized as a Bun/Turbo monorepo with a shared template core and explicit override zones for downstream forks. The shared paths live in `storefront/`, `backend/`, `tooling/`, and `docs/`; site-specific customization is meant to live in `packages/site-config/`, `storefront/site/`, and `backend/src/site/`.
